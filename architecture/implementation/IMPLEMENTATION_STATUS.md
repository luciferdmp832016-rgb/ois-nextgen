...
| ABACUS_STAGING_POC | ABACUS_HOSTED_APP_CUSTOM_DOMAIN_DEPLOY_BLOCKED_FROM_VM | Stage 0F selected split app staging as safest topology, verified PR #1 final-head CI/artifacts, and documented manual Abacus staging steps. Stage 0I-R1 confirmed source bootstrap. Stage 0J confirmed Core API local boot on the Abacus VM. Stage 0K confirmed public preview proxy health for Core API only with mock-safe env. Stage 0L confirmed custom-domain hosted-app deploy cannot be completed from VM shell alone; owner-assisted hosted-app service registration is required. Console/PITS, DB-backed endpoints, storage and real AI provider use remain excluded. |
