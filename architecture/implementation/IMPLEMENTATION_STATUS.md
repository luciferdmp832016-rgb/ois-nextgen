...
| OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE | STAGE_2I_OIMA_PRODUCT_SHELL_BOUNDARY_HARDENING_READY | Stage 2H establishes OIMA as a distinct Powered by OIS meeting intelligence product with key `OIMA`, product type `MEETING_INTELLIGENCE_PRODUCT`, transcript-first/audio-optional source modes, safety boundaries, OIS Core reuse map, deterministic `/platform/oima/*` contract endpoints, `/platform/products/code/OIMA` product projection and OIS Console `/oima`. Stage 2H-R1/R2 fix staging smoke marker drift. Stage 2I / OIMA-0 hardens the OIMA product shell with explicit `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, product boundary metadata and seven empty-state product surfaces for Meeting Library, Upload Meeting, Agent Analysis, Clarification Review, Dashboard, Self-Improvement Center and Listener Mode. It uses ADR `0006`, versioned migration `202607090003_stage_2h_oima_product_boundary`, adds no new migration or seed and keeps OIMA out of meeting upload/storage/transcript/audio/listener/LLM runtime scope. |
...
| OIMA_MEETING_INTELLIGENCE | PRODUCT_SHELL_HARDENED | Stage 2H adds the OIMA product boundary, source-mode taxonomy, safety boundary, roadmap, API projections and OIS Console shell. Stage 2I / OIMA-0 hardens the visible product shell and API boundary metadata while keeping Meeting Intake as the next recommended stage. Meeting upload, transcript persistence, transcript processing, audio processing, Listener Mode runtime, voice clone and LLM-backed analysis remain future stages. |
