...
| ABACUS_STAGING_POC | DEFAULT_DB_PRISMA_BASELINE_APPLIED | Stage 0F selected split app staging as safest topology, verified PR #1 final-head CI/artifacts, and documented manual Abacus staging steps. Stage 0I-R1 confirmed source bootstrap. Stage 0J confirmed Core API local boot on the Abacus VM. Stage 0K confirmed public preview proxy health. Stage 0L confirmed hosted-app deploy cannot be completed from VM shell alone. Stage 0N pivoted to SuperComputer nginx/systemd, Stage 0O verified Core API `/health`, and Stage 0P applied the Prisma baseline to `default` DB and verified DB-backed read-only `/platform/overview` on the Abacus-managed public staging domain. |
