...
| ABACUS_STAGING_POC | ABACUS_CORE_API_PREVIEW_PUBLIC_HEALTH_VERIFIED | Stage 0F selected split app staging as safest topology, verified PR #1 final-head CI/artifacts, and documented manual Abacus staging steps. Stage 0I-R1 confirmed source bootstrap. Stage 0J confirmed Core API local boot on the Abacus VM. Stage 0K confirmed public preview proxy health for Core API only with mock-safe env and no DB/storage/AI real credentials. Hosted-app/custom-domain routing, Always-On behavior, Console/PITS, DB-backed endpoints, storage and real AI provider use remain excluded. |
