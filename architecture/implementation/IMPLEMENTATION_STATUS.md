...
| ABACUS_STAGING_POC | ABACUS_CORE_API_LOCAL_BOOT_VERIFIED_PUBLIC_MAPPING_BLOCKED | Stage 0F selected split app staging as safest topology, verified PR #1 final-head CI/artifacts, and documented manual Abacus staging steps. Stage 0F-R3/R4 established the owner checklist and repo/local facts. Stage 0I-R1 confirmed source bootstrap. Stage 0J confirmed Core API local boot on the Abacus VM with mock-safe env and no DB/storage/AI real credentials, but public URL routing remains blocked and Console/PITS were not started. |
