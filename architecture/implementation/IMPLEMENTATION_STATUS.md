...
| OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE | STAGE_2H_OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE_ARCHITECTURE_READY | Stage 2H establishes OIMA as a distinct Powered by OIS meeting intelligence product with key `OIMA`, product type `MEETING_INTELLIGENCE_PRODUCT`, transcript-first/audio-optional source modes, safety boundaries, OIS Core reuse map, deterministic `/platform/oima/*` contract endpoints, `/platform/products/code/OIMA` product projection and OIS Console `/oima`. It uses ADR `0006`, versioned migration `202607090003_stage_2h_oima_product_boundary`, keeps OIMA out of meeting upload/storage/audio/LLM runtime scope and preserves no direct canonical knowledge writes. |
...
| OIMA_MEETING_INTELLIGENCE | PRODUCT_BOUNDARY_READY | Stage 2H adds the OIMA product boundary, source-mode taxonomy, safety boundary, roadmap, API projections and OIS Console shell. Meeting upload, transcript persistence, audio processing, Listener Mode and LLM-backed analysis remain future stages. |
