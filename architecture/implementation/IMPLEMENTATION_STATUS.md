...
| OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE | STAGE_2H_R2_OIMA_OVERVIEW_UNIVERSAL_KNOWLEDGE_API_MARKER_HOTFIX_READY | Stage 2H establishes OIMA as a distinct Powered by OIS meeting intelligence product with key `OIMA`, product type `MEETING_INTELLIGENCE_PRODUCT`, transcript-first/audio-optional source modes, safety boundaries, OIS Core reuse map, deterministic `/platform/oima/*` contract endpoints, `/platform/products/code/OIMA` product projection and OIS Console `/oima`. Stage 2H-R1 makes the OIMA product projection expose `displayName`, makes the overview contract expose `OIS Universal Knowledge API` explicitly and updates staging smoke helpers to validate minimum seeded overview/root counts instead of stale Stage 0Q exact counts. Stage 2H-R2 separates the exact `Universal Knowledge API` display name from the broader `OIS Universal Knowledge API` label so public/local smoke checks verify truthful explicit fields instead of a brittle quoted substring. It uses ADR `0006`, versioned migration `202607090003_stage_2h_oima_product_boundary`, keeps OIMA out of meeting upload/storage/audio/LLM runtime scope and preserves no direct canonical knowledge writes. |
...
| OIMA_MEETING_INTELLIGENCE | PRODUCT_BOUNDARY_READY | Stage 2H adds the OIMA product boundary, source-mode taxonomy, safety boundary, roadmap, API projections and OIS Console shell. Stage 2H-R1 fixes staging count/display marker drift and Stage 2H-R2 fixes the explicit Universal Knowledge API display-name marker contract. Meeting upload, transcript persistence, audio processing, Listener Mode and LLM-backed analysis remain future stages. |
