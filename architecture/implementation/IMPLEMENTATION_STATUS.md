...
| CANONICAL_KNOWLEDGE_FABRIC_KEIHB_LAYER_INTEGRATION | STAGE_2G_R1_KNOWLEDGE_FABRIC_ENDPOINT_MARKER_CONTRACT_HOTFIX_READY | Stage 2G adds the Canonical Knowledge Fabric and KEIHB projection foundation: KL-0 through KL-5 taxonomy, canonical knowledge items, evidence links, Learning Candidate to Knowledge Layer mappings, KEIHB bundle projections, deterministic knowledge context routes, OIS `/knowledge-fabric`, Learning Center integration and the architecture mindmap manifest. Stage 2G-R1 fixes the endpoint marker contract by exposing global `knowledgeLayerTaxonomy` / `availableLayers` metadata on knowledge context, Learning layer mapping and KEIHB bundle responses while preserving scoped result truthfulness and per-bundle `includedLayerKeys`. It uses ADR `0005`, versioned migration `202607090002_stage_2g_canonical_knowledge_fabric_keihb`, local non-production DB validation and keeps auto-promotion disabled. |
