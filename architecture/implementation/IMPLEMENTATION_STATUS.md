...
| OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE | STAGE_2K_OIMA_TRANSCRIPT_PROCESSING_FOUNDATION_READY | Stage 2H establishes OIMA as a distinct Powered by OIS meeting intelligence product with key `OIMA`, product type `MEETING_INTELLIGENCE_PRODUCT`, transcript-first/audio-optional source modes, safety boundaries, OIS Core reuse map, deterministic `/platform/oima/*` contract endpoints, `/platform/products/code/OIMA` product projection and OIS Console `/oima`. Stage 2H-R1/R2 fix staging smoke marker drift. Stage 2I / OIMA-0 hardens the OIMA product shell with explicit `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, product boundary metadata and seven product surfaces. Stage 2J / OIMA-1 adds audited meeting intake records and source file metadata registration through `/platform/oima/meetings`, `/platform/oima/meetings/:id`, `/platform/oima/meetings/:id/source-files`, `/platform/oima/meetings/:id/status`, plus OIS Console `/oima/meetings`, `/oima/meetings/new` and `/oima/meetings/[id]`. Stage 2K / OIMA-2 adds immutable raw transcript versions, separate normalized transcript versions, deterministic parse runs, ordered segments, warnings and transcript processing UI through `/platform/oima/transcripts/contract` plus `/platform/oima/meetings/:id/transcript/*`. It uses ADR `0008`, versioned migration `202607090005_stage_2k_oima_transcript_processing_foundation`, adds no seed data and keeps audio processing/listener/voice clone/LLM/meeting analytics/issue-decision-action-risk extraction out of runtime scope. |
...
| OIMA_MEETING_INTELLIGENCE | TRANSCRIPT_PROCESSING_FOUNDATION_READY | Stage 2H adds the OIMA product boundary, source-mode taxonomy, safety boundary, roadmap, API projections and OIS Console shell. Stage 2I / OIMA-0 hardens the visible product shell and API boundary metadata. Stage 2J / OIMA-1 persists workspace-scoped meeting records and source file metadata for transcript-first intake with optional audio metadata. Stage 2K / OIMA-2 persists immutable raw transcript versions, separate normalized transcript versions, parse runs, ordered segments and parse warnings for deterministic transcript processing. Audio processing, Listener Mode runtime, voice clone, fake meeting analytics, issue/decision/action/risk extraction and LLM-backed analysis remain future stages. |
