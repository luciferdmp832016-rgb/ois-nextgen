...
Stage 0W-A verdict: `PUBLIC_STAGING_RUNTIME_HARDENING_READY`

Stage 0W-B runtime verdict: `PUBLIC_STAGING_RUNTIME_OPERATIONAL`

Stage 0W-B hotfix verdict: `PUBLIC_STAGING_RUNTIME_SECRET_SAFE_HOTFIX_READY`

Stage 1A verdict: `PRODUCT_SHELL_NAVIGATION_BASELINE_READY`

Stage 1A-R1 verdict: `PRODUCT_SHELL_ROUTE_404_HOTFIX_READY`

Stage 1A-R2 verdict: `UI_ORPHAN_PORT_CLEANUP_READY`

Stage 1B verdict: `READONLY_PLATFORM_REGISTRY_API_DATA_BINDING_READY`

Stage 1C verdict: `PRODUCT_REGISTRY_DETAIL_CROSS_LINKING_READY`

Stage 1C-R1 verdict: `PRODUCT_REGISTRY_DETAIL_UI_MARKER_HOTFIX_READY`

Stage 1D verdict: `REGISTRY_RUNTIME_HEALTH_OWNER_UAT_READY`

Stage 1E verdict: `REGISTRY_GOVERNANCE_INSTALLATION_LIFECYCLE_READY`

Stage 1F verdict: `OWNER_REGISTRY_COCKPIT_VISUAL_UAT_READY`

Stage 1F-R1 verdict: `OWNER_REGISTRY_COCKPIT_ROOT_MARKER_HOTFIX_READY`

Stage 1G verdict: `MODERN_RESPONSIVE_SHELL_NAVIGATION_STANDARD_READY`

Stage 1H verdict: `OWNER_FIRST_VISUAL_DESIGN_SYSTEM_POLISH_READY`

Stage 1I verdict: `OWNER_REVIEW_WORKFLOW_SAFE_ACTION_BOUNDARY_READY`

Stage 1J verdict: `AUDIT_TRAIL_ADMIN_PERMISSION_MODEL_READY`

Stage 1K verdict: `PRODUCT_USER_JOURNEY_UAT_BASELINE_READY`

Stage 2A verdict: `PITS_PROJECT_WORKBOARD_READONLY_FUNCTIONAL_SLICE_READY`

Stage 2B verdict: `PITS_WORK_ITEM_DETAIL_DRY_RUN_ACTION_PREVIEW_READY`

Stage 2C verdict: `PRODUCT_UX_BLUEPRINT_SCREEN_FLOW_DRAFT_READY`

Stage 2D verdict: `LOCALIZATION_FOUNDATION_PRODUCT_FLOW_PREVIEW_READY`

Stage 2D-R1 verdict: `LOCALIZATION_COVERAGE_FONT_RUNTIME_MARKER_HOTFIX_READY`

Stage 2E verdict: `PITS_ACTION_REQUEST_AUDIT_SAFE_WRITE_BOUNDARY_READY`

Stage 2F verdict: `OIS_AGENT_RUNTIME_SELF_IMPROVEMENT_FOUNDATION_READY`

Stage 2G verdict: `CANONICAL_KNOWLEDGE_FABRIC_KEIHB_LAYER_INTEGRATION_READY`

Stage 2G-R1 verdict: `STAGE_2G_R1_KNOWLEDGE_FABRIC_ENDPOINT_MARKER_CONTRACT_HOTFIX_READY`

Stage 2G-R2 verdict: `STAGE_2G_R2_KNOWLEDGE_LAYER_MAPPING_STATUS_TAXONOMY_HOTFIX_READY`

Stage 2H verdict: `STAGE_2H_OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE_ARCHITECTURE_READY`

Stage 2H-R1 verdict: `STAGE_2H_R1_STAGING_SMOKE_CONTRACT_HOTFIX_READY`

Stage 2H-R2 verdict: `STAGE_2H_R2_OIMA_OVERVIEW_UNIVERSAL_KNOWLEDGE_API_MARKER_HOTFIX_READY`

Stage 2I / OIMA-0 verdict: `STAGE_2I_OIMA_PRODUCT_SHELL_BOUNDARY_HARDENING_READY`

Stage 2J / OIMA-1 verdict: `STAGE_2J_OIMA_MEETING_INTAKE_FOUNDATION_READY`

Stage 2K / OIMA-2 verdict: `STAGE_2K_OIMA_TRANSCRIPT_PROCESSING_FOUNDATION_READY`

Stage 2L / OIMA-A0 verdict: `STAGE_2L_STANDALONE_OIMA_APP_SHELL_DOMAIN_FOUNDATION_READY`
...
| PUBLIC_STAGING_RUNTIME_HARDENING | PUBLIC_STAGING_RUNTIME_HARDENING_READY | Stage 0W-A adds safe owner-run ops scripts for durable public staging: OIS Console and PITS Shell systemd unit install/uninstall, public staging restart/status, public endpoint checks and shared helpers. The UI services use ports 3000/3001, `CORE_API_URL=https://ois-nextgen.abacusai.cloud`, `NEXT_PUBLIC_CORE_API_URL=https://ois-nextgen.abacusai.cloud`, `NEXT_TELEMETRY_DISABLED=1`, and no `DATABASE_URL`. `runtime-sync.sh` can optionally restart Core API plus UI services with `PUBLIC_STAGING_RESTART_SCOPE=all`; cloudflared is not restarted unless explicitly requested. No Codex deploy, Cloudflare dashboard/DNS change, migration, seed, `prisma db push`, credential commit or legacy touch occurred. |
| PUBLIC_STAGING_RUNTIME_OPERATION | PUBLIC_STAGING_RUNTIME_OPERATIONAL | Stage 0W-A owner execution on Abacus passed after stopping legacy temporary UI demo processes and restarting durable OIS/PITS systemd services. Core API, OIS Console, PITS Shell and cloudflared were active; `https://ois-ng.dmp247.com`, `/dashboard`, `https://pits-ng.dmp247.com` and `/projects` all passed; `check-public-staging-endpoints.sh` passed. |
| PUBLIC_STAGING_RUNTIME_HOTFIX | PUBLIC_STAGING_RUNTIME_SECRET_SAFE_HOTFIX_READY | Stage 0W-B fixes public staging ops after runtime verification. `status-public-staging-runtime.sh` no longer prints full `systemctl status cloudflared`; it uses `systemctl is-active` and selected `systemctl show` properties only, avoiding ExecStart, process command lines and tunnel tokens. `restart-public-staging-runtime.sh` now stops legacy temporary UI demo processes before OIS/PITS systemd restart and prints safe port diagnostics for 3000/3001. |
| PRODUCT_SHELL_NAVIGATION_BASELINE | PRODUCT_SHELL_NAVIGATION_BASELINE_READY | Stage 1A updates OIS Console and PITS Shell from demo/status pages into product shell navigation baselines. OIS Console now has `/`, `/dashboard`, `/products`, `/workspaces` and `/runtime`; PITS Shell now has `/`, `/projects` and `/runtime`. Both shells use shared Core API snapshot helpers, show staging/demo banners, Core API source indicators, runtime status and seeded Platform Kernel counts through Core API only. Public endpoint scripts are updated for the new routes; public verification requires the next owner runtime sync. |
| PRODUCT_SHELL_ROUTE_404_HOTFIX | PRODUCT_SHELL_ROUTE_404_HOTFIX_READY | Stage 1A Abacus runtime verification found local/public HTTP 404s for new OIS `/products`, `/workspaces`, `/runtime` and PITS `/runtime` routes while root/dashboard/projects and Core API remained healthy. Stage 1A-R1 confirms route source files and local production build manifests contain the routes, then hardens Abacus ops by cleaning generated UI `.next` folders before build and verifying route manifests/server entries before service restart. Owner runtime re-sync is still required to prove public HTTP 200. |
| UI_ORPHAN_PORT_CLEANUP | UI_ORPHAN_PORT_CLEANUP_READY | Stage 1A-R2 records owner/manual Abacus evidence that Stage 1A-R1 route artifacts passed but orphan/legacy Next.js processes were holding ports `3000` and `3001`, blocking OIS/PITS systemd restart. Manual `systemctl stop`, `fuser -k 3000/tcp 3001/tcp`, `reset-failed` and `start` restored OIS/PITS services and verified all local/public Stage 1A routes. The permanent restart script now stops UI services, detects/cleans only ports 3000/3001, resets failed state, starts UI services, verifies local routes, then verifies public endpoints. |
| READONLY_PLATFORM_REGISTRY_API_DATA_BINDING | READONLY_PLATFORM_REGISTRY_API_DATA_BINDING_READY | Stage 1B adds read-only Core API Platform Registry endpoints for products, workspaces, projects, modules, installations and an aggregate registry snapshot. OIS Console and PITS Shell now bind product/workspace/project/module/installation pages to Core API registry data instead of hardcoded cards, while UI packages still avoid Prisma and `DATABASE_URL`. Public staging ops now check the new registry endpoints and registry-aware route markers after owner runtime sync. |
| PRODUCT_REGISTRY_DETAIL_CROSS_LINKING | PRODUCT_REGISTRY_DETAIL_CROSS_LINKING_READY | Stage 1C adds read-only Core API detail endpoints for products, product code lookup, workspaces, projects, modules and installations. OIS Console adds product/workspace/module/installation detail pages, PITS Shell adds project detail pages, and shared UI helpers build safe staging cross-links between `https://ois-ng.dmp247.com` and `https://pits-ng.dmp247.com`. Public staging ops now discover seeded IDs, verify detail APIs, controlled 404s and dynamic UI detail routes after owner runtime sync. |
| PRODUCT_REGISTRY_DETAIL_UI_MARKER_HOTFIX | PRODUCT_REGISTRY_DETAIL_UI_MARKER_HOTFIX_READY | Stage 1C-R1 fixes the runtime verification marker contract after owner Abacus evidence showed Core API detail endpoints, route manifests, systemd services, cloudflared and root/list/runtime pages passed, but local/public UI detail route checks failed because HTML responses lacked expected markers. OIS/PITS detail pages now render explicit visible `data-detail-source` markers while preserving read-only Core API-only behavior. |
| REGISTRY_RUNTIME_HEALTH_OWNER_UAT | REGISTRY_RUNTIME_HEALTH_OWNER_UAT_READY | Stage 1D adds read-only registry runtime health via Core API `/platform/registry/health`, OIS/PITS owner-facing health panels and deterministic ops markers for configured, linked, reachable, missing URL, not applicable, degraded and unavailable states. |
| REGISTRY_GOVERNANCE_INSTALLATION_LIFECYCLE | REGISTRY_GOVERNANCE_INSTALLATION_LIFECYCLE_READY | Stage 1E adds read-only deterministic registry readiness via Core API `/platform/registry/readiness`, owner-facing OIS/PITS governance panels and ops checks for ready, incomplete, blocked, not applicable and unknown readiness states. Owner runtime sync and browser/UAT must pass before closing `REGISTRY_GOVERNANCE_INSTALLATION_LIFECYCLE_RUNTIME_VERIFIED`. |
| OWNER_REGISTRY_COCKPIT_VISUAL_UAT | OWNER_REGISTRY_COCKPIT_VISUAL_UAT_READY | Stage 1F adds owner-facing OIS/PITS cockpit and visual UAT navigation surfaces that summarize existing registry counts, health, readiness, missing link/runtime URL guard status, forbidden link guard status and cross-product detail links. It adds no Core API endpoint and remains read-only. |
| OWNER_REGISTRY_COCKPIT_ROOT_MARKER_HOTFIX | OWNER_REGISTRY_COCKPIT_ROOT_MARKER_HOTFIX_READY | Stage 1F-R1 fixes the OIS Console root marker contract by rendering deterministic server-side `Ready to operate` text on `/` without weakening ops checks or changing the real cockpit status. |
| MODERN_RESPONSIVE_SHELL_NAVIGATION_STANDARD | MODERN_RESPONSIVE_SHELL_NAVIGATION_STANDARD_READY | Stage 1G standardizes OIS Console and PITS Shell around a shared modern responsive shell with fixed navigation/header, independent main-content scrolling, hide/show controls, responsive drawer behavior and deterministic shell markers. It changes no Core API endpoint or registry data behavior. |
| OWNER_FIRST_VISUAL_DESIGN_SYSTEM_POLISH | OWNER_FIRST_VISUAL_DESIGN_SYSTEM_POLISH_READY | Stage 1H polishes owner-facing OIS/PITS information architecture, visual hierarchy, status badges, empty/fallback states and responsive readability while preserving existing Core API, registry, health, readiness, cockpit, link-boundary and ops marker behavior. |
| OWNER_REVIEW_WORKFLOW_SAFE_ACTION_BOUNDARY | OWNER_REVIEW_WORKFLOW_SAFE_ACTION_BOUNDARY_READY | Stage 1I adds read-only owner review workflow boundary data via `/platform/owner-review` plus OIS/PITS safe action-boundary surfaces. It derives review items from existing health/readiness data and adds no mutation endpoint, admin execution, schema change, migration, seed, `prisma db push`, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| AUDIT_TRAIL_ADMIN_PERMISSION_MODEL | AUDIT_TRAIL_ADMIN_PERMISSION_MODEL_READY | Stage 1J adds read-only audit/admin permission model data via `/platform/admin-boundary` plus OIS/PITS admin boundary surfaces. It exposes roles, permission states, action categories, audit requirements, confirmation requirements, rollback requirements, blocked actions and preview-only future actions without adding mutation endpoints, admin execution, schema change, migration, seed, `prisma db push`, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| PRODUCT_USER_JOURNEY_UAT_BASELINE | PRODUCT_USER_JOURNEY_UAT_BASELINE_READY | Stage 1K adds read-only product user journey UAT baseline data via `/platform/product-uat` plus OIS/PITS functional gap map surfaces. It distinguishes browser-testable surfaces, platform/control-plane-only surfaces, placeholder/shell-only surfaces, future product functions and blockers without adding mutation endpoints, product writes, schema changes, migrations, seed, `prisma db push`, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| PITS_PROJECT_WORKBOARD_READONLY_FUNCTIONAL_SLICE | PITS_PROJECT_WORKBOARD_READONLY_FUNCTIONAL_SLICE_READY | Stage 2A adds the first read-only PITS user-level functional slice via `/platform/pits/projects/{id}/workboard` plus PITS `/projects/{id}/workboard` and project-detail workboard UI. It uses deterministic demo/runtime data, grouped status columns and read-only work item cards without adding mutation endpoints, task lifecycle writes, schema changes, migrations, seed, `prisma db push`, auth changes, credentials, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` changes or legacy-resource touch. |
| PITS_WORK_ITEM_DETAIL_DRY_RUN_ACTION_PREVIEW | PITS_WORK_ITEM_DETAIL_DRY_RUN_ACTION_PREVIEW_READY | Stage 2B adds read-only PITS work item detail and dry-run action preview via `/platform/pits/projects/{projectId}/work-items/{itemId}` and `/platform/pits/projects/{projectId}/work-items/{itemId}/action-preview` plus PITS `/projects/{id}/work-items/{itemId}`. It exposes deterministic preview-only action metadata with `noDataChanged=true` and adds no mutation endpoint, task lifecycle write, schema change, migration, seed, `prisma db push`, auth change, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| PRODUCT_UX_BLUEPRINT_SCREEN_FLOW_DRAFT | PRODUCT_UX_BLUEPRINT_SCREEN_FLOW_DRAFT_READY | Stage 2C adds OIS and PITS product UX blueprints, the product/admin separation map and read-only `/product-flow` preview routes for both apps. It is an owner approval draft gate before Stage 2D and adds no Core API endpoint, mutation endpoint, real product write, LLM call, schema change, migration, seed, `prisma db push`, auth change, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| LOCALIZATION_FOUNDATION_PRODUCT_FLOW_PREVIEW | LOCALIZATION_FOUNDATION_PRODUCT_FLOW_PREVIEW_READY | Stage 2D adds shared English/Tiếng Việt localization helpers, a shell language selector and visual OIS/PITS Product Flow Preview pages. Localization is presentation-only; stable IDs, internal status codes, routes, API contracts and ops markers remain deterministic. It adds no Core API endpoint, mutation endpoint, real product write, LLM call, schema change, migration, seed, `prisma db push`, auth change, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change or legacy-resource touch. |
| LOCALIZATION_COVERAGE_FONT_RUNTIME_MARKER_HOTFIX | LOCALIZATION_COVERAGE_FONT_RUNTIME_MARKER_HOTFIX_READY | Stage 2D-R1 restores the stable language-neutral `Core API source:` runtime marker, expands visible English/Tiếng Việt localization coverage, adds read-only OIS/PITS `/localization` catalog routes and applies Vietnamese-safe font/line-height safeguards. It adds no Core API endpoint, mutation endpoint, browser editing persistence, schema change, migration, seed, `prisma db push`, auth change, credential, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change, DNS/Cloudflare change or legacy-resource touch. |
| PITS_ACTION_REQUEST_AUDIT_SAFE_WRITE_BOUNDARY | PITS_ACTION_REQUEST_AUDIT_SAFE_WRITE_BOUNDARY_READY | Stage 2E adds read-only PITS action request list/detail/preview contracts for work items plus a PITS Work Item Detail action-request panel. It stages deterministic request intent with `noDirectMutation=true`, audit/confirmation/rollback/permission gates and source-item unchanged evidence without adding mutation endpoints, uncontrolled writes, schema changes, migrations, seed, `prisma db push`, auth changes, credentials, UI `DATABASE_URL`, Prisma UI import, `/auth/demo-login` change, DNS/Cloudflare change or legacy-resource touch. |
| OIS_AGENT_RUNTIME_SELF_IMPROVEMENT_FOUNDATION | OIS_AGENT_RUNTIME_SELF_IMPROVEMENT_FOUNDATION_READY | Stage 2F adds the governed OIS Agent Runtime and Self-Improvement Engine foundation: Powered by OIS product registry, learning scopes/policies, Learning Signals, Learning Candidates, Agent sessions/messages/feedback/submissions, SuperAdmin Learning Center, OIS Agent Widget and audit-backed learning submission routes. It uses ADR `0004`, versioned migration `202607090001_stage_2f_agent_runtime_self_improvement`, local non-production DB validation and keeps widget/candidate paths from writing canonical Knowledge Layer records. |
| CANONICAL_KNOWLEDGE_FABRIC_KEIHB_LAYER_INTEGRATION | STAGE_2G_R2_KNOWLEDGE_LAYER_MAPPING_STATUS_TAXONOMY_HOTFIX_READY | Stage 2G adds the Canonical Knowledge Fabric and KEIHB projection foundation: KL-0 through KL-5 taxonomy, canonical knowledge items, evidence links, Learning Candidate to Knowledge Layer mappings, KEIHB bundle projections, deterministic knowledge context routes, OIS `/knowledge-fabric`, Learning Center integration and the architecture mindmap manifest. Stage 2G-R1 fixes the endpoint marker contract by exposing global `knowledgeLayerTaxonomy` / `availableLayers` metadata on knowledge context, Learning layer mapping and KEIHB bundle responses while preserving scoped result truthfulness and per-bundle `includedLayerKeys`. Stage 2G-R2 exposes `mappingStatusTaxonomy` / `availableStatuses` on Learning layer mapping responses so smoke checks verify allowed statuses instead of requiring current rows to contain `READY_FOR_REVIEW`. It uses ADR `0005`, versioned migration `202607090002_stage_2g_canonical_knowledge_fabric_keihb`, local non-production DB validation and keeps auto-promotion disabled. |
| OIMA_PRODUCT_BOUNDARY_MEETING_INTELLIGENCE | STAGE_2L_STANDALONE_OIMA_APP_SHELL_DOMAIN_FOUNDATION_READY | Stage 2H establishes OIMA as a distinct Powered by OIS meeting intelligence product with key `OIMA`, product type `MEETING_INTELLIGENCE_PRODUCT`, transcript-first/audio-optional source modes, safety boundaries, OIS Core reuse map, deterministic `/platform/oima/*` contract endpoints, `/platform/products/code/OIMA` product projection and OIS Console `/oima`. Stage 2H-R1/R2 fix staging smoke marker drift. Stage 2I / OIMA-0 hardens the OIMA product shell with explicit `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, product boundary metadata and seven product surfaces. Stage 2J / OIMA-1 adds audited meeting intake records and source file metadata registration through `/platform/oima/meetings`, `/platform/oima/meetings/:id`, `/platform/oima/meetings/:id/source-files` and `/platform/oima/meetings/:id/status`. Stage 2K / OIMA-2 adds immutable raw transcript versions, separate normalized transcript versions, deterministic parse runs, ordered segments, warnings and transcript processing UI through `/platform/oima/transcripts/contract` plus `/platform/oima/meetings/:id/transcript/*`. Stage 2L / OIMA-A0 moves the OIMA browser runtime to standalone app `apps/oima-shell` (`@ois/oima-shell`), service `ois-nextgen-oima-staging`, port `3002`, custom domain foundation `https://oima.dmp247.com`, while OIS Console `/oima*` routes become launcher/compatibility routes. It adds no schema, migration, seed data, DB silo, duplicate Core API, audio processing, Listener Mode, voice clone, LLM/OpenRouter call, fake analytics or issue/decision/action/risk extraction. |
| ABACUS_STAGING_POC | PRODUCT_USER_JOURNEY_UAT_BASELINE_READY | Stage 0F selected split app staging as safest topology, verified PR #1 final-head CI/artifacts, and documented manual Abacus staging steps. Stage 0I-R1 confirmed source bootstrap. Stage 0J confirmed Core API local boot on the Abacus VM. Stage 0K confirmed public preview proxy health. Stage 0L confirmed hosted-app deploy cannot be completed from VM shell alone. Stage 0N pivoted to SuperComputer nginx/systemd, Stage 0O verified Core API `/health`, Stage 0P applied the Prisma baseline to `default` DB, Stage 0Q applied demo/staging Platform Kernel seed data, Stage 0R-A confirmed gate logic, Stage 0R-B added focused local-only test coverage, Stage 0R-C synced Abacus runtime with no behavior regression, Stage 0R-D created safe SSH operations, Stage 0R-E documented the SSH relay blocker plus Web Terminal fallback path, Stage 0R-F added restart grace-window retries, Stage 0R-G fixed the ops helper unbound-variable crash, Stage 0S-A prepared two UI shell demos, Stage 0S-B added temporary preview ops scripts, Stage 0T-A corrected UI deployment to Apps Management Console App Shells, Stage 0T-B added local/Codex UI demo test harness coverage, Stage 0T-C deployed OIS Console as its own App Shell, Stage 0T-D-R1 prepared the PITS Shell direct upload source bundle after GitHub clone was blocked, Stage 0T-D-R2 deployed PITS Shell from that bundle, Stage 0T-E-A-R1 prepared the OIS Console direct upload bundle for restoration, Stage 0U-A verified local SuperComputer product subdomain routing while public custom subdomains were blocked at Abacus TLS/edge registration, Stage 0V-A planned Cloudflare Tunnel, Stage 0V-B/C verified Cloudflare Tunnel public HTTPS for OIS/PITS product staging subdomains, Stage 0W-A prepared durable systemd-backed public staging operations, Stage 0W-B records operational runtime evidence plus a secret-safe ops hotfix, Stage 1A prepares real product-shell navigation baselines, Stage 1A-R1 prepares the route 404 hotfix, Stage 1A-R2 makes orphan UI port cleanup permanent, Stage 1B adds read-only Platform Registry API data binding, Stage 1C adds read-only registry detail APIs plus OIS/PITS cross-product linking, Stage 1C-R1 fixes the detail UI marker contract for runtime verification, Stage 1D adds owner-verifiable registry runtime health surfaces, Stage 1E adds owner-verifiable registry governance/readiness surfaces, Stage 1F adds owner-verifiable cockpit and visual UAT navigation surfaces, Stage 1F-R1 fixes the OIS root `Ready to operate` marker contract, Stage 1G adds the modern responsive shell/navigation standard, Stage 1H adds owner-first IA and visual design polish, Stage 1I adds read-only owner review workflow and safe action-boundary preview surfaces, Stage 1J adds read-only audit/admin permission model surfaces, and Stage 1K adds read-only product user journey UAT baseline and functional gap map surfaces. |
...
| KNOWLEDGE_VERTICAL_SLICE | CANONICAL_KNOWLEDGE_FABRIC_FOUNDATION_READY | Stage 2G adds KL-0 through KL-5, canonical knowledge item/evidence contracts, candidate-to-layer mapping, KEIHB projections and a universal knowledge read contract. Stage 2G-R1 standardizes global taxonomy metadata on read/projection endpoints and Stage 2G-R2 standardizes mapping status taxonomy metadata without enabling promotion. Promotion remains blocked until a later ADR, rollback tests and owner-approved gates exist. |
| OIMA_MEETING_INTELLIGENCE | STANDALONE_APP_SHELL_READY | Stage 2H adds the OIMA product boundary, source-mode taxonomy, safety boundary, roadmap, API projections and OIS Console shell. Stage 2I / OIMA-0 hardens the visible product shell and API boundary metadata. Stage 2J / OIMA-1 persists workspace-scoped meeting records and source file metadata for transcript-first intake with optional audio metadata. Stage 2K / OIMA-2 persists immutable raw transcript versions, separate normalized transcript versions, parse runs, ordered segments and parse warnings for deterministic transcript processing. Stage 2L / OIMA-A0 makes OIMA a standalone frontend/runtime shell on port `3002` with `oima.dmp247.com` Cloudflare Tunnel foundation and keeps OIS Console as a launcher. Audio processing, Listener Mode runtime, voice clone, fake meeting analytics, issue/decision/action/risk extraction and LLM-backed analysis remain future stages. |
