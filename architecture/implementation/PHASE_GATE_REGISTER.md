...
| GitHub CI activation | Stage 0E checkpoint | BLOCKED | PR #1 exists, but no Actions run exists for the PR head because Stage 0D pull-request trigger omitted the repository default base branch. Stage 0E patches the trigger; a push is required before CI artifacts can be verified. |
