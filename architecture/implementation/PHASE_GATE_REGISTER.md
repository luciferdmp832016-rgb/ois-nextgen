...
| Abacus staging runtime POC | Stage 0F checkpoint | BLOCKED_STAGING_INPUTS | Split app staging is the recommended topology. Stage 0F-R2 confirms the runtime POC cannot execute safely because staging DB, storage/mock mode, AI mock config, Abacus service identifiers, env/secrets injection path, runtime port behavior and staging URLs remain unknown or missing. No staging or production deployment was executed. |
