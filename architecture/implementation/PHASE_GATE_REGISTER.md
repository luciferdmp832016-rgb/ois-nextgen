...
| Abacus staging runtime POC | Stage 0F checkpoint | BLOCKED_STAGING_INPUTS | Split app staging is the recommended topology. Stage 0F-R2 confirms the runtime POC cannot execute safely because staging DB, storage/mock mode, AI mock config, Abacus service identifiers, env/secrets injection path, runtime port behavior and staging URLs remain unknown or missing. Stage 0F-R3 adds a redacted owner acquisition checklist; runtime POC remains blocked until all required staging-only inputs and approvals are confirmed. No staging or production deployment was executed. |
