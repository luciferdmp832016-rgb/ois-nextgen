...
| Stage 1F | `https://ois-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Source-ready owner cockpit root marker; not deployed from Codex. | Expected HTTP 200 with `Owner Registry Cockpit / Registry Runtime Summary` after owner runtime sync. |
...
| OIS Console root cockpit after Stage 1F sync | `curl -i https://ois-ng.dmp247.com` | HTTP 200 with `Owner Registry Cockpit / Registry Runtime Summary`. |
