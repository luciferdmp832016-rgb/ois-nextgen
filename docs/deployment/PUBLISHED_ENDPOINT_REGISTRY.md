...
| `PUBLIC_STAGING_RUNTIME_OPERATIONAL` | Durable Core API, OIS Console, PITS Shell, OIMA Shell and Cloudflare Tunnel runtime operation is verified; endpoint status remains unchanged. |
...
| OIS Console OIMA launcher | `https://ois-ng.dmp247.com/oima` | `PLANNED_NOT_CREATED` | HTTP 200 launcher/compatibility route pointing to the standalone OIMA app at `https://oima.dmp247.com`; not an independent OIMA runtime implementation. | Stage 2L OIS Console launcher smoke check. |
| OIS Console OIMA Meeting Library launcher | `https://ois-ng.dmp247.com/oima/meetings` | `PLANNED_NOT_CREATED` | HTTP 200 launcher/compatibility route pointing to `https://oima.dmp247.com/meetings`. | Stage 2L OIS Console launcher smoke check. |
| OIS Console OIMA Upload/Register Meeting launcher | `https://ois-ng.dmp247.com/oima/meetings/new` | `PLANNED_NOT_CREATED` | HTTP 200 launcher/compatibility route pointing to `https://oima.dmp247.com/meetings/new`. | Stage 2L OIS Console launcher smoke check. |
| OIS Console OIMA Meeting Detail launcher | `https://ois-ng.dmp247.com/oima/meetings/{meeting-id}` | `PLANNED_NOT_CREATED` | HTTP 200 launcher/compatibility route pointing to `https://oima.dmp247.com/meetings/{meeting-id}`. | Stage 2L real-record launcher UI check after meeting registration. |
...
| OIMA Shell Cloudflare Tunnel staging | `https://oima.dmp247.com` | `PLANNED_CLOUDFLARE_TUNNEL` | HTTP 200 standalone OIMA app shell with `OIMA_APP_SHELL`, `OIMA_STANDALONE_APP_SHELL`, `STAGE_2L_STANDALONE_OIMA_APP_SHELL`, `OIMA_DOMAIN_READY`, Core API source and seeded counts after Stage 2L runtime sync. | Stage 2L OIMA standalone root smoke check. |
| OIMA Shell Meeting Library | `https://oima.dmp247.com/meetings` | `PLANNED_CLOUDFLARE_TUNNEL` | HTTP 200 standalone OIMA Meeting Library using Stage 2J Meeting Intake API through Core API only. | Stage 2L OIMA meeting library smoke check. |
| OIMA Shell Upload/Register Meeting | `https://oima.dmp247.com/meetings/new` | `PLANNED_CLOUDFLARE_TUNNEL` | HTTP 200 standalone OIMA meeting intake form with transcript-first validation and optional audio metadata registration. | Stage 2L OIMA meeting registration smoke check. |
| OIMA Shell Meeting Detail | `https://oima.dmp247.com/meetings/{meeting-id}` | `PLANNED_CLOUDFLARE_TUNNEL` | HTTP 200 standalone OIMA meeting detail and transcript processing surface for a real registered meeting. | Stage 2L real-record detail UI check after meeting registration. |
| OIMA Shell planned surfaces | `https://oima.dmp247.com/analysis`, `/clarification`, `/dashboard`, `/self-improvement`, `/listener` | `PLANNED_CLOUDFLARE_TUNNEL` | HTTP 200 planned/not-runtime placeholders with no LLM, audio, Listener Mode or voice clone runtime. | Stage 2L planned route smoke checks. |
...
Stage 2L / OIMA-A0 adds standalone OIMA app shell endpoints at `https://oima.dmp247.com`, `/meetings`, `/meetings/new`, `/meetings/{meeting-id}`, `/analysis`, `/clarification`, `/dashboard`, `/self-improvement` and `/listener`. It changes OIS Console `/oima*` routes into launcher/compatibility surfaces pointing at the standalone app. It adds no Core API endpoint, schema migration, seed data, data silo, audio processing, Listener Mode, voice clone or LLM-backed analysis.
...
| OIS Console OIMA launcher after Stage 2L sync | `curl -i https://ois-ng.dmp247.com/oima` | HTTP 200 with `OIMA Console Launcher`, `Open OIMA standalone app`, `OIMA_APP_SHELL`, `OIMA_STANDALONE_APP_SHELL`, `STAGE_2L_STANDALONE_OIMA_APP_SHELL`, `Product Runtime separate from Product Administration` and `https://oima.dmp247.com/`. |
| OIS Console OIMA Meeting Library launcher after Stage 2L sync | `curl -i https://ois-ng.dmp247.com/oima/meetings` | HTTP 200 with `Meeting Library moved to OIMA app`, `Open OIMA Meeting Library`, `OIMA Console Launcher` and `https://oima.dmp247.com/meetings`. |
| OIS Console OIMA Upload/Register Meeting launcher after Stage 2L sync | `curl -i https://ois-ng.dmp247.com/oima/meetings/new` | HTTP 200 with `Upload Meeting moved to OIMA app`, `Open OIMA Upload / Register Meeting`, `OIMA Console Launcher` and `https://oima.dmp247.com/meetings/new`. |
| OIMA Shell root after Stage 2L sync | `curl -i https://oima.dmp247.com` | HTTP 200 with `OIMA_APP_SHELL`, `Standalone OIMA App Shell`, `OIMA_DOMAIN_READY`, `OIMA Product Surface Empty States`, `Platform Overview Counts`, Core API source and no `DATABASE_URL`. |
| OIMA Shell Meeting Library after Stage 2L sync | `curl -i https://oima.dmp247.com/meetings` | HTTP 200 with `OIMA Meeting Intake`, `Meeting Library`, `Register meeting`, `Source mode`, `Transcript present`, `Audio present`, `LISTENER_CAPTURED planned/not-runtime`, `No fake meeting analysis`, `OIMA-2 available` and `MEETING_INTAKE`. |
| OIMA Shell Upload/Register Meeting after Stage 2L sync | `curl -i https://oima.dmp247.com/meetings/new` | HTTP 200 with `Upload / Register Meeting`, `Transcript-first registration`, `TRANSCRIPT_ONLY supported`, `TRANSCRIPT_AND_AUDIO optional audio`, `Optional Audio Source`, `No voice clone` and `No LLM/OpenRouter calls`. |
| OIMA Shell Meeting Detail after Stage 2L registration | `curl -i https://oima.dmp247.com/meetings/<meeting-id>` | HTTP 200 for a real registered meeting with `Meeting Detail`, `Source Files`, `Transcript Processing`, `RAW transcript immutable`, `NORMALIZED transcript separate`, `Transcript Timeline`, `Parse Warnings`, planned analysis/listener placeholders and no fake analytics. |
