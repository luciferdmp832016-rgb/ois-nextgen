...
| OIS Console SuperComputer product subdomain root | `https://ois-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Stage 0U-A scripts ready; requires owner-run UI demo shells, nginx route enablement, DNS CNAME and TLS/edge verification. Do not touch `ois.dmp247.com`. |
| PITS Shell SuperComputer product subdomain root | `https://pits-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Stage 0U-A scripts ready; requires owner-run UI demo shells, nginx route enablement, DNS CNAME and TLS/edge verification. |
| OIS Console SuperComputer product subdomain dashboard | `https://ois-ng.dmp247.com/dashboard` | `PLANNED_NOT_CREATED` | Stage 0U-A planned route; verify only after root route and DNS/TLS are configured. |
| PITS Shell SuperComputer product subdomain projects | `https://pits-ng.dmp247.com/projects` | `PLANNED_NOT_CREATED` | Stage 0U-A planned route; verify only after root route and DNS/TLS are configured. |
...
| Stage 0U-A | `https://ois-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Planned SuperComputer nginx host-based product subdomain route. | After owner DNS/nginx/TLS: OIS Console root demo with `OIS_CONSOLE`, shared Core API URL and seeded counts. |
| Stage 0U-A | `https://pits-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Planned SuperComputer nginx host-based product subdomain route. | After owner DNS/nginx/TLS: PITS Shell root demo with `PITS_SHELL`, shared Core API URL and seeded counts. |
| Stage 0U-A | `https://ois-ng.dmp247.com/dashboard` | `PLANNED_NOT_CREATED` | Planned OIS Console dashboard route through the same host. | HTTP 200 after owner DNS/nginx/TLS succeeds. |
| Stage 0U-A | `https://pits-ng.dmp247.com/projects` | `PLANNED_NOT_CREATED` | Planned PITS Shell projects route through the same host. | HTTP 200 after owner DNS/nginx/TLS succeeds. |
...
| OIS product subdomain root | `https://ois-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Stage 0U-A scripts are ready; do not test publicly until owner DNS/TLS is configured. Local pre-DNS check uses `curl -H "Host: ois-ng.dmp247.com" http://127.0.0.1/` on Abacus. |
| PITS product subdomain root | `https://pits-ng.dmp247.com` | `PLANNED_NOT_CREATED` | Stage 0U-A scripts are ready; do not test publicly until owner DNS/TLS is configured. Local pre-DNS check uses `curl -H "Host: pits-ng.dmp247.com" http://127.0.0.1/` on Abacus. |
| OIS product subdomain dashboard | `https://ois-ng.dmp247.com/dashboard` | `PLANNED_NOT_CREATED` | Verify only after owner DNS/TLS and root product route pass. |
| PITS product subdomain projects | `https://pits-ng.dmp247.com/projects` | `PLANNED_NOT_CREATED` | Verify only after owner DNS/TLS and root product route pass. |
