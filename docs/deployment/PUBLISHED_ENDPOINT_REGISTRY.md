...
| OIS NextGen OIMA overview API | `https://ois-nextgen.abacusai.cloud/platform/oima/overview` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA overview with transcript-first/audio-optional rules, safety boundaries and OIS Core reuse map after Stage 2H runtime sync. | Stage 2H OIMA overview smoke check. |
...
| OIMA overview API after Stage 2H-R1 sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/overview` | HTTP 200 with source modes, safety boundaries, capability codes, OIS Core reuse map, `OIS Universal Knowledge API` and no LLM/runtime meeting processing claim. |
