...
| OIS NextGen OIMA product code API | `https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA product boundary projection with `MEETING_INTELLIGENCE_PRODUCT`, `PRODUCT_BOUNDARY_READY` and no fake installations after Stage 2H runtime sync. | Stage 2H OIMA product registry smoke check. |
| OIS NextGen OIMA overview API | `https://ois-nextgen.abacusai.cloud/platform/oima/overview` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA overview with transcript-first/audio-optional rules, safety boundaries, OIS Core reuse map and explicit Universal Knowledge API display metadata after Stage 2H-R2 runtime sync. | Stage 2H OIMA overview smoke check validates `universalKnowledgeApiDisplayName` and `universalKnowledgeApiLabel`. |
...
| OIS Console OIMA product boundary | `https://ois-ng.dmp247.com/oima` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA product boundary shell with product boundary, source modes, safety boundary, OIS Core reuse, roadmap and placeholder product surfaces after Stage 2H runtime sync. | Stage 2H OIMA UI smoke check. |
...
| OIMA roadmap API after Stage 2H sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/roadmap` | HTTP 200 with `Stage 2H`, `PRODUCT_BOUNDARY_READY`, `OIMA-0`, `OIMA-9` and planned meeting intake/transcript/listener stages. |
...
| OIS OIMA shell after Stage 2H sync | `curl -i https://ois-ng.dmp247.com/oima` | HTTP 200 with `OIMA`, `Product Boundary`, `Source Mode Contract`, `Safety Boundary`, `OIS Core Reuse Map`, `OIMA Roadmap`, `OIMA-0`, `OIMA-9`, `TRANSCRIPT_ONLY` and `PRODUCT_BOUNDARY_READY`. |
