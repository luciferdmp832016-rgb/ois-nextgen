...
| OIS NextGen OIMA product code API | `https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA product boundary projection with `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, `MEETING_INTAKE`, `MEETING_INTELLIGENCE_PRODUCT`, `PRODUCT_BOUNDARY_READY` and no fake installations after Stage 2J runtime sync. | Stage 2J OIMA product registry smoke check. |
| OIS NextGen OIMA overview API | `https://ois-nextgen.abacusai.cloud/platform/oima/overview` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA overview with Stage 2J/OIMA-1 product shell metadata, transcript-first/audio-optional intake rules, current/planned runtime capabilities, empty-state surfaces, safety boundaries, OIS Core reuse map and explicit Universal Knowledge API display metadata after runtime sync. | Stage 2J OIMA overview smoke check validates `productCode`, `productName`, `currentRuntimeCapabilities`, `MEETING_INTAKE`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, `universalKnowledgeApiDisplayName` and `universalKnowledgeApiLabel`. |
| OIS NextGen OIMA source modes API | `https://ois-nextgen.abacusai.cloud/platform/oima/source-modes` | `PLANNED_NOT_CREATED` | HTTP 200 source mode taxonomy including `TRANSCRIPT_ONLY`, `AUDIO_ONLY`, `TRANSCRIPT_AND_AUDIO`, `LISTENER_CAPTURED`, transcript-primary mode metadata and listener planned/not-runtime metadata. | Stage 2J source-mode smoke check. |
| OIS NextGen OIMA roadmap API | `https://ois-nextgen.abacusai.cloud/platform/oima/roadmap` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA roadmap with `Stage 2J`, `RUNTIME_FOUNDATION_READY`, `OIMA-1`, `OIMA-2` and future transcript/listener stages. | Stage 2J roadmap smoke check. |
| OIS NextGen OIMA boundary API | `https://ois-nextgen.abacusai.cloud/platform/oima/boundary` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA safety boundary with meeting intake enabled, transcript/audio/listener/voice-clone/LLM runtimes disabled, no canonical write, no auto-promotion and no impersonation markers. | Stage 2J boundary smoke check. |
| OIS NextGen OIMA meetings API | `https://ois-nextgen.abacusai.cloud/platform/oima/meetings` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA Meeting Intake list payload with `intakeContract`, `meetingStatuses`, `sourceFileTypes`, `uploadStatuses`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `READY_FOR_PROCESSING`, `TRANSCRIPT`, `AUDIO`, `noFakeMeetingAnalysis=true` and `noLlmCalls=true`. Meeting rows may truthfully be empty until a user registers a meeting. | Stage 2J Meeting Intake contract smoke check. |
| OIS NextGen OIMA meeting detail API | `https://ois-nextgen.abacusai.cloud/platform/oima/meetings/{meeting-id}` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA Meeting Detail payload for a real registered meeting with metadata, source files, transcript/audio availability and planned next-step placeholders; no seeded fake detail row is required. | Stage 2J real-record detail check after meeting registration. |
...
| OIS Console OIMA product boundary | `https://ois-ng.dmp247.com/oima` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA product shell with OIMA overview, product boundary, source modes, safety boundary, OIS Core reuse, roadmap, OIMA-1 intake ready status and truthful product surface empty states after Stage 2J runtime sync. | Stage 2J OIMA UI smoke check. |
| OIS Console OIMA Meeting Library | `https://ois-ng.dmp247.com/oima/meetings` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA Meeting Library table with title, meeting date, source mode, status, transcript/audio presence, created date and open-detail action; empty table is valid before user registration. | Stage 2J Meeting Library UI smoke check. |
| OIS Console OIMA Upload/Register Meeting | `https://ois-ng.dmp247.com/oima/meetings/new` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA meeting intake form with transcript-first validation, optional audio metadata registration, listener planned/not-runtime copy and no fake meeting analysis. | Stage 2J meeting creation UI smoke check. |
| OIS Console OIMA Meeting Detail | `https://ois-ng.dmp247.com/oima/meetings/{meeting-id}` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA detail route for a real registered meeting, including source files, status and planned Transcript Processing, OIS Agent Analysis, Clarification Review and Dashboard placeholders. | Stage 2J real-record detail UI check after meeting registration. |
...
Stage 2J / OIMA-1 adds source-ready OIMA Meeting Intake endpoints: `/platform/oima/meetings`, `/platform/oima/meetings/{meeting-id}`, `/platform/oima/meetings/{meeting-id}/source-files` and `/platform/oima/meetings/{meeting-id}/status`. It also adds OIS `/oima/meetings`, `/oima/meetings/new` and `/oima/meetings/{meeting-id}`. Public verification is pending owner runtime sync, versioned migration deploy and browser/UAT; transcript processing, audio processing, Listener Mode, voice clone and LLM-backed analysis remain future stages.
...
| OIMA product code API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | HTTP 200 with `displayName`, `OIMA`, `MEETING_INTELLIGENCE_PRODUCT`, `MEETING_INTAKE`, `PRODUCT_BOUNDARY_READY`, `TRANSCRIPT_ONLY`, `NO_LIVE_SPEAKING_AGENT` and `OIS Canonical Knowledge Fabric`. |
| OIMA overview API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/overview` | HTTP 200 with `productCode=OIMA`, `productName=Organizational Intelligence Meeting Agent`, source modes, `MEETING_INTAKE`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, safety boundaries, OIS Core reuse map, `universalKnowledgeApiDisplayName="Universal Knowledge API"`, `universalKnowledgeApiLabel="OIS Universal Knowledge API"` and no LLM/runtime meeting processing claim. |
| OIMA source modes API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/source-modes` | HTTP 200 with `TRANSCRIPT_ONLY`, `AUDIO_ONLY`, `TRANSCRIPT_AND_AUDIO`, `LISTENER_CAPTURED`, `primarySourceMode=TRANSCRIPT_ONLY`, transcript-first guidance, optional audio metadata and listener planned/not-runtime markers. |
| OIMA roadmap API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/roadmap` | HTTP 200 with `Stage 2J`, `RUNTIME_FOUNDATION_READY`, `OIMA-0`, `OIMA-1`, `OIMA-2`, `OIMA-9` and planned transcript/listener stages. |
| OIMA boundary API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/boundary` | HTTP 200 with `meetingIntakeImplemented=true`, `transcriptProcessingImplemented=false`, no canonical write, no auto-promotion, no live speaking agent, no voice clone, no impersonation and no autonomous decision markers. |
| OIMA meetings API after Stage 2J sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/meetings` | HTTP 200 with `intakeContract`, `meetingStatuses`, `sourceFileTypes`, `uploadStatuses`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `AUDIO_ONLY`, `READY_FOR_PROCESSING`, `TRANSCRIPT`, `AUDIO`, `noFakeMeetingAnalysis=true` and `noLlmCalls=true`. Meeting rows may be empty until real registration. |
| OIS OIMA shell after Stage 2J sync | `curl -i https://ois-ng.dmp247.com/oima` | HTTP 200 with `OIMA Product Overview`, `OIMA-1 intake ready`, `Open Meeting Library`, `Product Boundary`, `Source Mode Contract`, `Safety Boundary`, `OIS Core Reuse Map`, `OIMA Roadmap`, `OIMA Product Surface Empty States`, `Meeting Library`, `Upload Meeting`, `Available now`, `Agent Analysis`, `Clarification Review`, `Self-Improvement Center`, `Listener Mode`, `Planned / not runtime`, `No fake meeting data`, `TRANSCRIPT_ONLY` and `MEETING_INTAKE`. |
| OIS OIMA Meeting Library after Stage 2J sync | `curl -i https://ois-ng.dmp247.com/oima/meetings` | HTTP 200 with `OIMA Meeting Intake`, `Meeting Library`, `Register meeting`, `Source mode`, `Transcript present`, `Audio present`, `LISTENER_CAPTURED planned/not-runtime`, `No fake meeting analysis` and `MEETING_INTAKE`. |
| OIS OIMA Upload/Register Meeting after Stage 2J sync | `curl -i https://ois-ng.dmp247.com/oima/meetings/new` | HTTP 200 with `Register OIMA Meeting`, `Transcript-first intake`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `Optional audio metadata`, `LISTENER_CAPTURED planned/not-runtime` and `No fake meeting analysis`. |
| OIS OIMA Meeting Detail after Stage 2J registration | `curl -i https://ois-ng.dmp247.com/oima/meetings/<meeting-id>` | HTTP 200 for a real registered meeting with `Meeting Detail`, `Source Files`, `Transcript Processing`, `OIS Agent Analysis`, `Clarification Review`, `Dashboard`, `Planned / not runtime` and no fake analytics. |
