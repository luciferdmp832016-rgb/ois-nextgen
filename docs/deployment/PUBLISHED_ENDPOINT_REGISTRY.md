...
| OIS NextGen OIMA product code API | `https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA product boundary projection with `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, `MEETING_INTELLIGENCE_PRODUCT`, `PRODUCT_BOUNDARY_READY` and no fake installations after Stage 2I runtime sync. | Stage 2I OIMA product registry smoke check. |
| OIS NextGen OIMA overview API | `https://ois-nextgen.abacusai.cloud/platform/oima/overview` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA overview with Stage 2I/OIMA-0 product shell metadata, transcript-first/audio-optional rules, current/planned runtime capabilities, empty-state surfaces, safety boundaries, OIS Core reuse map and explicit Universal Knowledge API display metadata after runtime sync. | Stage 2I OIMA overview smoke check validates `productCode`, `productName`, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, `universalKnowledgeApiDisplayName` and `universalKnowledgeApiLabel`. |
| OIS NextGen OIMA source modes API | `https://ois-nextgen.abacusai.cloud/platform/oima/source-modes` | `PLANNED_NOT_CREATED` | HTTP 200 source mode taxonomy including `TRANSCRIPT_ONLY`, `AUDIO_ONLY`, `TRANSCRIPT_AND_AUDIO` and `LISTENER_CAPTURED`. | Stage 2H source-mode smoke check. |
| OIS NextGen OIMA roadmap API | `https://ois-nextgen.abacusai.cloud/platform/oima/roadmap` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA roadmap with `PRODUCT_BOUNDARY_READY` and future meeting-intake stages. | Stage 2H roadmap smoke check. |
| OIS NextGen OIMA boundary API | `https://ois-nextgen.abacusai.cloud/platform/oima/boundary` | `PLANNED_NOT_CREATED` | HTTP 200 OIMA safety boundary with no canonical write, no auto-promotion and no live speaking/voice clone/impersonation markers. | Stage 2H boundary smoke check. |
...
Stage 2H adds source-ready OIMA product boundary endpoints: `/platform/products/code/OIMA`, `/platform/oima/overview`, `/platform/oima/source-modes`, `/platform/oima/roadmap` and `/platform/oima/boundary`. It also adds OIS `/oima`. Public verification is pending owner runtime sync, versioned migration deploy and browser/UAT; meeting upload, transcript persistence, audio processing, Listener Mode and LLM-backed analysis remain future stages.
...
| OIMA product code API after Stage 2H-R1 sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | HTTP 200 with `displayName`, `OIMA`, `MEETING_INTELLIGENCE_PRODUCT`, `PRODUCT_BOUNDARY_READY`, `TRANSCRIPT_ONLY`, `NO_LIVE_SPEAKING_AGENT` and `OIS Canonical Knowledge Fabric`. |
| OIMA overview API after Stage 2I sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/overview` | HTTP 200 with `productCode=OIMA`, `productName=Organizational Intelligence Meeting Agent`, source modes, `currentRuntimeCapabilities`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, safety boundaries, OIS Core reuse map, `universalKnowledgeApiDisplayName="Universal Knowledge API"`, `universalKnowledgeApiLabel="OIS Universal Knowledge API"` and no LLM/runtime meeting processing claim. |
| OIMA source modes API after Stage 2H sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/source-modes` | HTTP 200 with `TRANSCRIPT_ONLY`, `AUDIO_ONLY`, `TRANSCRIPT_AND_AUDIO`, `LISTENER_CAPTURED`, `primarySourceMode=TRANSCRIPT_ONLY` and `audioDoesNotBlockAnalysis=true`. |
| OIMA roadmap API after Stage 2I sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/roadmap` | HTTP 200 with `Stage 2I`, `PRODUCT_SHELL_HARDENED`, `OIMA-0`, `OIMA-1`, `OIMA-9` and planned meeting intake/transcript/listener stages. |
| OIMA boundary API after Stage 2H sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/boundary` | HTTP 200 with no canonical write, no auto-promotion, no live speaking agent, no voice clone, no impersonation and no autonomous decision markers. |
| OIS OIMA shell after Stage 2I sync | `curl -i https://ois-ng.dmp247.com/oima` | HTTP 200 with `OIMA Product Overview`, `OIMA-0 shell ready`, `Product Boundary`, `Source Mode Contract`, `Safety Boundary`, `OIS Core Reuse Map`, `OIMA Roadmap`, `OIMA Product Surface Empty States`, `Meeting Library`, `Upload Meeting`, `Agent Analysis`, `Clarification Review`, `Self-Improvement Center`, `Listener Mode`, `Planned / not runtime`, `No fake meeting data`, `TRANSCRIPT_ONLY` and `PRODUCT_BOUNDARY_READY`. |
