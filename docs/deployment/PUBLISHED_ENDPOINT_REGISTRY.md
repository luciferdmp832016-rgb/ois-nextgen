...
| OIS NextGen knowledge context API | `https://ois-nextgen.abacusai.cloud/platform/knowledge/context` | `PLANNED_NOT_CREATED` | HTTP 200 deterministic knowledge context with no LLM call and no canonical write after Stage 2G runtime sync. | Stage 2G knowledge context smoke check. |
| OIS NextGen KEIHB bundle API | `https://ois-nextgen.abacusai.cloud/platform/knowledge/keihb/bundles` | `PLANNED_NOT_CREATED` | HTTP 200 KEIHB projection bundles over OIS Knowledge Fabric after Stage 2G runtime sync. | Stage 2G KEIHB projection smoke check. |
...
Stage 2G adds source-ready OIS Canonical Knowledge Fabric and KEIHB projection endpoints: `/platform/knowledge/layers`, `/platform/knowledge/items`, `/platform/knowledge/evidence`, `/platform/knowledge/context`, `/platform/knowledge/read-contract/example`, `/platform/learning/layer-mappings`, `/platform/knowledge/keihb/bundles`, `/platform/knowledge/keihb/preview`, `/platform/agent/knowledge-context` and `/platform/architecture/mindmap`. It also adds OIS `/knowledge-fabric` and Learning Center knowledge-layer integration. Public verification is pending owner runtime sync, versioned migration deploy and browser/UAT; auto-promotion remains disabled.
...
| Knowledge context API after Stage 2G sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/knowledge/context` | HTTP 200 with `deterministic-knowledge-context`, `noLlmCall=true`, `noCanonicalWrite=true` and `autoPromotionEnabled=false`. |
| Knowledge layer mappings API after Stage 2G sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/learning/layer-mappings` | HTTP 200 with candidate-to-layer mappings and `READY_FOR_REVIEW`. |
| KEIHB bundles API after Stage 2G sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/knowledge/keihb/bundles` | HTTP 200 with `KEIHB`, `projectionBoundary` and `sourceOfTruth` as OIS Knowledge Fabric. |
