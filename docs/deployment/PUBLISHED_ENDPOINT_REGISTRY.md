...
| OIMA product code API after Stage 2H sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/products/code/OIMA` | HTTP 200 with `OIMA`, `MEETING_INTELLIGENCE_PRODUCT`, `PRODUCT_BOUNDARY_READY`, `TRANSCRIPT_ONLY`, `NO_LIVE_SPEAKING_AGENT` and `OIS Canonical Knowledge Fabric`. |
| OIMA overview API after Stage 2H sync | `curl -i https://ois-nextgen.abacusai.cloud/platform/oima/overview` | HTTP 200 with source modes, safety boundaries, capability codes, OIS Core reuse map and no LLM/runtime meeting processing claim. |
