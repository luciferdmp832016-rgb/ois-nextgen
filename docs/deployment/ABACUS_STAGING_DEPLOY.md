...
- Stage 2H-R2 fixes the remaining OIMA overview marker contract by exposing `universalKnowledgeApiDisplayName: "Universal Knowledge API"` separately from `universalKnowledgeApiLabel: "OIS Universal Knowledge API"` and making smoke checks validate those explicit fields. It adds no migration, seed, meeting runtime, upload, audio, Listener Mode or LLM behavior.
...
- Core API `/platform/products/code/OIMA`, `/platform/oima/overview`, `/platform/oima/source-modes`, `/platform/oima/roadmap` and `/platform/oima/boundary` return OIMA product boundary payloads after owner runtime sync and migration deploy. After Stage 2K, `/platform/oima/overview` must include `productCode`, `productName`, `currentRuntimeCapabilities`, `MEETING_INTAKE`, `TRANSCRIPT_PROCESSING`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, `universalKnowledgeApiDisplayName: "Universal Knowledge API"` and `universalKnowledgeApiLabel: "OIS Universal Knowledge API"`.
- Core API `/platform/oima/meetings` returns the OIMA Meeting Intake list payload after Stage 2J owner runtime sync and migration deploy. The response must include `intakeContract`, `meetingStatuses`, `sourceFileTypes`, `uploadStatuses`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `AUDIO_ONLY`, `READY_FOR_PROCESSING`, `TRANSCRIPT`, `AUDIO`, `noFakeMeetingAnalysis=true` and `noLlmCalls=true`; meeting rows may truthfully be empty before user registration.
- Core API `/platform/oima/transcripts/contract` returns the OIMA Transcript Processing contract after Stage 2K owner runtime sync and migration deploy. The response must include `Stage 2K / OIMA-2`, `TRANSCRIPT_PROCESSING`, `RAW`, `NORMALIZED`, `CORRECTED`, `MICROSOFT_TEAMS`, `GENERIC_TEXT`, `NEEDS_REVIEW`, `rawTranscriptImmutable=true`, `normalizedTranscriptSeparate=true`, `rawTranscriptOverwriteAllowed=false`, `audioProcessingImplemented=false`, `meetingAnalyticsImplemented=false`, `issueDecisionActionRiskExtractionImplemented=false` and `realLlmCallsEnabled=false`.
- OIS `/oima` renders `OIMA Product Overview`, `OIMA-2 transcript ready`, `Open Meeting Library`, `Product Boundary`, `Source Mode Contract`, `Safety Boundary`, `OIS Core Reuse Map`, `OIMA Roadmap`, `OIMA Product Surface Empty States`, available-now Meeting Library/Upload Meeting/Transcript Processing markers, planned/not-runtime analysis/listener markers, `TRANSCRIPT_ONLY`, `MEETING_INTELLIGENCE_PRODUCT`, `MEETING_INTAKE` and `TRANSCRIPT_PROCESSING` after Stage 2K owner runtime sync.
- OIS `/oima/meetings` renders `OIMA Meeting Intake`, `Meeting Library`, `Register meeting`, `Source mode`, `Transcript present`, `Audio present`, `LISTENER_CAPTURED planned/not-runtime`, `No fake meeting analysis`, `MEETING_INTAKE` and `OIMA-2 available` after Stage 2K owner runtime sync.
- OIS `/oima/meetings/new` renders `Register OIMA Meeting`, `Transcript-first intake`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `Optional audio metadata`, `LISTENER_CAPTURED planned/not-runtime`, `Transcript processing available after registration`, `No audio processing`, `No OIS Agent analysis`, `No voice clone` and `No LLM/OpenRouter calls` after Stage 2K owner runtime sync.
...
Stage 2H-R2 adds no migration and no seed requirement. Redeploy Core API plus the updated ops scripts, then rerun public/local smoke scripts to verify the explicit Universal Knowledge API display-name contract.
Stage 2I / OIMA-0 adds no migration and no seed requirement. Redeploy Core API/OIS Console plus updated ops scripts, then rerun public/local smoke scripts to verify the hardened OIMA product shell and boundary metadata.
Stage 2J / OIMA-1 requires applying versioned migration `202607090004_stage_2j_oima_meeting_intake_foundation` to the non-production staging database. It adds no seed requirement. Redeploy Core API/OIS Console plus updated ops scripts, then rerun public/local smoke scripts to verify OIMA Meeting Intake availability without requiring seeded meeting records.
