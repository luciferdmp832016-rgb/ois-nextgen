...
- Core API `/platform/products/code/OIMA`, `/platform/oima/overview`, `/platform/oima/source-modes`, `/platform/oima/roadmap` and `/platform/oima/boundary` return OIMA product boundary payloads after owner runtime sync and migration deploy. After Stage 2J, `/platform/oima/overview` must include `productCode`, `productName`, `currentRuntimeCapabilities`, `MEETING_INTAKE`, `plannedRuntimeCapabilities`, `emptyStateSurfaces`, `universalKnowledgeApiDisplayName: "Universal Knowledge API"` and `universalKnowledgeApiLabel: "OIS Universal Knowledge API"`.
...
- OIS `/oima` renders `OIMA Product Overview`, `OIMA-1 intake ready`, `Open Meeting Library`, `Product Boundary`, `Source Mode Contract`, `Safety Boundary`, `OIS Core Reuse Map`, `OIMA Roadmap`, `OIMA Product Surface Empty States`, available-now Meeting Library/Upload Meeting markers, planned/not-runtime analysis/listener markers, `TRANSCRIPT_ONLY`, `MEETING_INTELLIGENCE_PRODUCT` and `MEETING_INTAKE` after Stage 2J owner runtime sync.
- OIS `/oima/meetings` renders `OIMA Meeting Intake`, `Meeting Library`, `Register meeting`, `Source mode`, `Transcript present`, `Audio present`, `LISTENER_CAPTURED planned/not-runtime`, `No fake meeting analysis` and `MEETING_INTAKE` after Stage 2J owner runtime sync.
- OIS `/oima/meetings/new` renders `Register OIMA Meeting`, `Transcript-first intake`, `TRANSCRIPT_ONLY`, `TRANSCRIPT_AND_AUDIO`, `Optional audio metadata`, `LISTENER_CAPTURED planned/not-runtime` and `No fake meeting analysis` after Stage 2J owner runtime sync.
...
- An OIMA route enables binary file storage, transcript processing, audio processing, Listener Mode runtime, live speaking, voice clone, impersonation, autonomous decisions, fake analytics, real LLM/OpenRouter calls or direct canonical knowledge writes outside the explicit Stage 2J meeting/source-file metadata registration boundary.
