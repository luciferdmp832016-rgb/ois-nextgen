# OIS Console Abacus Upload Bundle

Stage 0T-E-A-R1 prepares an uploadable source bundle for restoring or redeploying OIS Console because the previously deployed OIS Console App Shell preview now returns HTTP 404.

This bundle uses the same source-upload strategy that succeeded for PITS Shell in Stage 0T-D-R2. It keeps the App Shell source tied to the OIS NextGen source-of-truth repo and does not include secrets or generated artifacts.
...
| Expected product code | `OIS_CONSOLE` |

## Dependency Inspection

OIS Console imports one local workspace package:

| Import | Required bundle path | Evidence |
|---|---|---|
| `@ois/shared-ui` | `packages/shared-ui/**` | `apps/ois-console/next.config.mjs` transpiles `@ois/shared-ui`; Console pages import `demoBannerText`. |

No direct Prisma, database client, Core API package, domain package or production/legacy package import is required for the OIS Console App Shell build.

## Included Paths

The upload bundle intentionally includes only the source and workspace metadata needed to build OIS Console:

| Path | Reason |
|---|---|
| `apps/ois-console/**` | OIS Console Next.js app source, package manifest, config and local tests. |
...
| `pnpm-workspace.yaml` | Workspace package discovery for `@ois/ois-console` and `@ois/shared-ui`. |
| `tsconfig.base.json` | Base TypeScript config extended by `apps/ois-console/tsconfig.json`. |
...
- env files exist under `apps/ois-console` or `packages/shared-ui`
...
git archive --format=zip --output artifacts/abacus/ois-console-abacus-upload-bundle.zip HEAD -- package.json pnpm-lock.yaml pnpm-workspace.yaml tsconfig.base.json apps/ois-console packages/shared-ui
```
...
2. Create or open `OIS NextGen Console Demo`.
3. Choose the direct source upload option.
4. Upload `artifacts/abacus/ois-console-abacus-upload-bundle.zip`.
5. Configure as a Next.js App Shell.
6. Configure safe environment values only.
7. Build and start using the commands below.
8. Verify the App Shell preview URL before two-App-Shell verification.
...
```sh
corepack enable || true && pnpm install --frozen-lockfile && pnpm --filter @ois/ois-console build
```

Start command:

```sh
pnpm --filter @ois/ois-console start
```
...
After Abacus builds and starts the uploaded OIS Console App Shell:

| Check | Expected |
|---|---|
| OIS Console App Shell URL | HTTP 200. |
| App Shell name | `OIS Console` visible. |
| Product code | `OIS_CONSOLE` visible. |
...
| PITS coexistence | Existing PITS Shell App Shell `https://113d93f4db-3001.na116.preview.abacusai.app` remains healthy. |
| Legacy boundaries | `https://oisys.abacusai.app`, `https://ois.dmp247.com`, `ois_phase1_dev` and `emerald_bql_web_dev` remain untouched. |

## Safety

Stage 0T-E-A-R1 does not deploy, modify Abacus runtime, run migrations, run seed, run `prisma db push`, call write endpoints, call `/auth/demo-login`, attach a `dmp247.com` domain or touch legacy resources.
