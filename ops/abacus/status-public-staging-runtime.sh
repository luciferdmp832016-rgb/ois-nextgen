...
check_service "$OIMA_SHELL_SERVICE" "OIMA_SHELL"
...
check_route "LOCAL_OIMA_OVERVIEW" "$CORE_API_LOCAL_BASE/platform/oima/overview" "${oima_endpoint_markers[@]}" '"OIS Canonical Knowledge Fabric"' '"universalKnowledgeApiDisplayName":"Universal Knowledge API"' '"universalKnowledgeApiLabel":"OIS Universal Knowledge API"' '"emptyStateSurfaces"' '"availableNow":true' '"runtimeEnabled":false'
...
check_route "LOCAL_OIMA_ROADMAP" "$CORE_API_LOCAL_BASE/platform/oima/roadmap" "${oima_endpoint_markers[@]}" '"roadmap"' '"Stage 2K"' '"TRANSCRIPT_PROCESSING_READY"' '"OIMA-0"' '"OIMA-1"' '"OIMA-2"' '"OIMA-9"'
check_route "LOCAL_OIMA_BOUNDARY" "$CORE_API_LOCAL_BASE/platform/oima/boundary" "${oima_endpoint_markers[@]}" '"noCanonicalKnowledgeWrite":true' '"autoPromotionEnabled":false' '"meetingIntakeImplemented":true' '"transcriptProcessingImplemented":true' '"rawTranscriptImmutable":true' '"normalizedTranscriptSeparate":true' '"meetingAnalyticsImplemented":false'
check_route "LOCAL_OIMA_MEETINGS" "$CORE_API_LOCAL_BASE/platform/oima/meetings" "${oima_meeting_intake_endpoint_markers[@]}" '"meetings"' '"count"'
check_route "LOCAL_OIMA_TRANSCRIPTS_CONTRACT" "$CORE_API_LOCAL_BASE/platform/oima/transcripts/contract" "${oima_transcript_endpoint_markers[@]}"
...
check_absent_markers "OIS_CONSOLE_LOCAL_OIMA_MEETINGS_LINK_BOUNDARY" "$OIS_CONSOLE_LOCAL_URL/oima/meetings" "localhost" "127.0.0.1" "ois.dmp247.com" "oisys.abacusai.app"
check_absent_markers "OIS_CONSOLE_LOCAL_OIMA_MEETINGS_NEW_LINK_BOUNDARY" "$OIS_CONSOLE_LOCAL_URL/oima/meetings/new" "localhost" "127.0.0.1" "ois.dmp247.com" "oisys.abacusai.app"
...
check_route "OIS_CONSOLE_LOCAL_OIMA" "$OIS_CONSOLE_LOCAL_URL/oima" "${oima_console_launcher_markers[@]}" "OIS_CONSOLE" "OIMA-0" "OIMA-1" "OIMA-2" "OIMA-9"
check_route "OIS_CONSOLE_LOCAL_OIMA_MEETINGS" "$OIS_CONSOLE_LOCAL_URL/oima/meetings" "${oima_console_launcher_markers[@]}" "Meeting Library moved to OIMA app" "Open OIMA Meeting Library" "OIS_CONSOLE"
check_route "OIS_CONSOLE_LOCAL_OIMA_MEETINGS_NEW" "$OIS_CONSOLE_LOCAL_URL/oima/meetings/new" "${oima_console_launcher_markers[@]}" "Upload Meeting moved to OIMA app" "Open OIMA Upload / Register Meeting" "OIS_CONSOLE"
...
check_root_shell "OIMA_SHELL_LOCAL" "$OIMA_SHELL_LOCAL_URL" "OIMA_APP_SHELL" "OIMA"
check_route "OIMA_SHELL_LOCAL_ROOT_CONTRACT" "$OIMA_SHELL_LOCAL_URL" "${oima_ui_markers[@]}" "Core API source:" "Platform Overview Counts" "DEMO DATA - NOT PRODUCTION"
check_route "OIMA_SHELL_LOCAL_MEETINGS" "$OIMA_SHELL_LOCAL_URL/meetings" "${oima_meeting_library_ui_markers[@]}" "OIMA_APP_SHELL"
check_route "OIMA_SHELL_LOCAL_MEETINGS_NEW" "$OIMA_SHELL_LOCAL_URL/meetings/new" "${oima_meeting_new_ui_markers[@]}" "OIMA_APP_SHELL"
check_route "OIMA_SHELL_LOCAL_ANALYSIS" "$OIMA_SHELL_LOCAL_URL/analysis" "${oima_planned_ui_markers[@]}" "Agent Analysis"
check_route "OIMA_SHELL_LOCAL_CLARIFICATION" "$OIMA_SHELL_LOCAL_URL/clarification" "${oima_planned_ui_markers[@]}" "Clarification Review"
check_route "OIMA_SHELL_LOCAL_DASHBOARD" "$OIMA_SHELL_LOCAL_URL/dashboard" "${oima_planned_ui_markers[@]}" "Dashboard"
check_route "OIMA_SHELL_LOCAL_SELF_IMPROVEMENT" "$OIMA_SHELL_LOCAL_URL/self-improvement" "${oima_planned_ui_markers[@]}" "Self-Improvement Center"
check_route "OIMA_SHELL_LOCAL_LISTENER" "$OIMA_SHELL_LOCAL_URL/listener" "${oima_planned_ui_markers[@]}" "Listener Mode"
check_absent_markers "OIMA_SHELL_LOCAL_ROOT_LINK_BOUNDARY" "$OIMA_SHELL_LOCAL_URL" "ois.dmp247.com" "oisys.abacusai.app"
check_absent_markers "OIMA_SHELL_LOCAL_MEETINGS_LINK_BOUNDARY" "$OIMA_SHELL_LOCAL_URL/meetings" "ois.dmp247.com" "oisys.abacusai.app"
