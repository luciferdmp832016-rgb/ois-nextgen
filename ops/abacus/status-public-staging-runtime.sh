...
check_route "OIS_CONSOLE_LOCAL_PRODUCT_FLOW" "$OIS_CONSOLE_LOCAL_URL/product-flow" "${product_flow_ui_markers[@]}" "OIS Product UX Blueprint" "Executive Dashboard" "Workspace Intelligence Dashboard" "Meeting/Document Knowledge Feed" "Knowledge Detail" "Ask OIS / Copilot" "Runtime/Admin" "No LLM call"
...
check_route "PITS_SHELL_LOCAL_PRODUCT_FLOW" "$PITS_SHELL_LOCAL_URL/product-flow" "${product_flow_ui_markers[@]}" "PITS Product UX Blueprint" "Home" "Projects" "Project Detail" "Workboard" "Work Item Detail" "Dry-run Action Preview" "Runtime and Admin" "No data will be changed"
