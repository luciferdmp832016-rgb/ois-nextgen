...
ZIP_NAME="${ZIP_NAME:-pits-shell-abacus-upload-bundle.zip}"
...
for source_path in apps/pits-shell packages/shared-ui; do
...
printf 'PITS shell upload bundle created\n'
printf 'path=%s\n' "$REPO_ROOT/$ZIP_PATH"
printf 'size_bytes=%s\n' "$(file_size_bytes "$ZIP_PATH")"
printf 'bundle_ref=%s\n' "$(git rev-parse "$BUNDLE_REF")"
printf '%s\n' "Upload this ZIP to the PITS Shell Abacus App Shell source upload flow."
